...
const Activities = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const completedRes = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/challenges/completed`, {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handleSubmission = async (e) => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/challenges/submit`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
        body: formData,
      });
  ...
  const handleCreateChallenge = async () => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/challenges/create`, {
        method: 'POST',
        headers: {
  ...
  const handleRemoveChallenge = async (id) => {
    const token = localStorage.getItem('token');
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/api/challenges/delete/${id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) {
